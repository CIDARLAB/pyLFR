...
INTRODUCTION:
    ...
    supported on bit vectors are:
            ...
            __iadd__               for adding to an existing bitvector
...
OTHER SUPPORTED OPERATIONS:
   ...
   (16) concatenation
        ...
        Starting with version 3.4.9, the module includes an implementation
        for __iadd__ that allows for the following compound assignment to
...
ACKNOWLEDGMENTS:
    ...
    The two changes made in Version 3.4.9 were supplied by Elliot James
    Edmunds. These include a new implementation for __iadd__, and a
    significantly faster implementation for __add__.  Thanks, Elliot!
    

@title
ABOUT THE AUTHOR:

    The author, Avinash Kak, not too long ago finished his 17-year long
    Objects Trilogy project with the publication of the book "Designing
    with Objects" by John-Wiley. If interested, check out his web page at
    Purdue to find out what the Objects Trilogy project was all about. You
    might like "Designing with Objects" especially if you enjoyed reading
...
class BitVector(object):
    def __init__(self, *args, **kwargs):
        ...
        filename = (
            ...
            intVal
        ) = size = bitlist = bitstring = textstring = hexstring = rawbytes = None
    ...
    def __iadd__(self, other):
        ...
        if not isinstance(other, type(self)):
            raise TypeError(
                "Can only join two BitVector objects, not {}".format(type(other))
            )
    ...
    def __lshift__(self, n):
        ...
        if self.size == 0:
            raise ValueError(
                """Circular shift of an empty vector
                                makes no sense"""
            )
        if n < 0:
            return self >> abs(n)
    ...
    def jaccard_similarity(self, other):
        ...
        """
        assert (
            self.intValue() > 0 or other.intValue() > 0
