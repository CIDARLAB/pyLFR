...
class FluidExpression:
            ...
            for operator in operatorset:
                ...
                while len(indices) > 0:
                    ...
                    if is_number(operand1) and is_number(operand2):
                        ...
                        # Update the indices
                        indices = [
                            i for i, x in enumerate(operatorlist) if x is operator
                        ]
            ...
            for operator in operatorset:
                ...
                while index >= 0:
                    ...
                    if is_number(operand1) and is_number(operand2):
                        ...
                        print(
                            "Performing operation: {0} {1} {2}".format(
    ...
    def __evalute_fluid_fluid_operator(
        ...
        if len(operand1) is not len(operand2):
            ...
            raise Exception(
                "Operand {0} and Operand {1} are of different Dimensions".format(
                    operand1, operand2
                )
            )
        ...
        for i in range(len(operand1)):
            ...
            if isinstance(operand1_element, Interaction) and isinstance(
                ...
                result_element = (
                    self.currentmodule.add_finteraction_finteraction_interaction(
    ...
    def __evaluate_fluid_numeric_operator(
        ...
        for fluid in operand_fluidic:
            ...
            if operator == "*":
                ...
                raise Exception(
                    "Unsuppored operator on 1:fluidic 2:numeric values: {0}".format(
                        operator
                    )
                )
            elif operator == "-":
